...
function doFilter() {
    ...
    const rows = playerRows.getElementsByTagName("tr");
    ...
    // Loop through the rows and show filtered rows
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const playerName = row.getElementsByTagName("td")[0].innerText.toLowerCase(); 
        const playerTeam = row.getElementsByTagName("td")[1].innerText.toLowerCase(); 

        const matchesName = playerName.includes(searchTerm);
        const matchesTeam = selectedTeam === "all" || playerTeam === selectedTeam.toLowerCase();

        // If matches both the name and the team show the row
